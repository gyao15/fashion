...
parser.add_argument('--resume', default='runs/Type_Specific_Fashion_Compatibility/origin/model_best.pth.tar', type=str,
...
parser.add_argument('--embed_loss', type=float, default=5e-2, metavar='M',
                    help='parameter for loss for embedding norm')
parser.add_argument('--mask_loss', type=float, default=5e-2, metavar='M',
                    help='parameter for loss for mask norm')
parser.add_argument('--vse_loss', type=float, default=5e-1, metavar='M',
                    help='parameter for loss for the visual-semantic embedding')
parser.add_argument('--sim_t_loss', type=float, default=5e-3, metavar='M',
                    help='parameter for loss for text-text similarity')
parser.add_argument('--sim_i_loss', type=float, default=5e-3, metavar='M',
...
def main():
    ...
    #tnet = mynet(args, csn_model, text_feature_dim)
    if args.cuda:
        tnet.cuda()
...
def train(train_loader, tnet, optimizer, epoch):
    ...
    for batch_idx, (img1, desc1, has_text1, img2, desc2, has_text2, img3, desc3, has_text3, condition) in enumerate(train_loader):
        ...
        loss_sim = args.sim_i_loss * loss_sim_i + args.sim_t_loss * loss_sim_t
        ...
        losses.update(loss_triplet.data.item(), num_items)
        accs.update(acc.data.item(), num_items)
        ...
        if batch_idx % args.log_interval == 0:
            end_time = time.time()
            print('Train Epoch: {} [{}/{}]\tLoss: {:.4f} ({:.4f})  Acc: {:.2f}% ({:.2f}%) \tEmb_Norm: {:.2f} ({:.2f})  Time: {:.2f}'.format(epoch, batch_idx * num_items, len(train_loader.dataset),losses.val, losses.avg, 100. * accs.val, 100. * accs.avg, emb_norms.val, emb_norms.avg,end_time-start_time))
            start_time = time.time()
