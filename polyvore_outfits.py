...
class TripletImageLoader(torch.utils.data.Dataset):
    def __init__(self, args, split, meta_data, text_dim = None, transform=None, loader=default_image_loader):
        ...
        if self.is_train:
            ...
            self.desc2vecs = {}
            featfile = os.path.join(rootdir, 't', 'train_hglmm_pca6000.txt')
    ...
    def sample_negative(self, outfit_id, item_id, item_type):
        ...
        while item_out == item_id and attempts < 100:
            ...
            item_index = np.random.choice(list(range(len(items))))
    ...
    def test_compatibility(self, embeds, metric):
        ...
        s = 0
        print('max subtract min:',max(scores)-min(scores))
        print('min:',min(scores))
        scores = (scores-min(scores))/(max(scores)-min(scores))
        
        f = open('result_compatibility.txt','w')
        for i in range(len(scores)):
            f.writelines(str(1-scores[i]))
            f.writelines('\n')
            if scores[i] >0.5 and labels[i]==0:
                s = s+1
            elif scores[i] <0.5 and labels[i] == 1:
                s = s+1
        f.close()
        m = s/len(scores)
        print('compatibility accuracy':,m)
    ...
    def test_fitb(self, embeds, metric):
        ...
        for q_index, (questions, answers, is_correct) in enumerate(self.fitb_questions):
            ...
            for index, (answer, img1) in enumerate(answers):
                ...
                score = (score/len(questions)-0.2)/1.3

                answer_score[index] = score.squeeze().cpu().numpy()
                f.writelines(1-str(answer_score[index])+" ")
